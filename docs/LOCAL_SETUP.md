...
This guide explains how to run the NYC Taxi ETL locally using Docker and Makefile commands.
...
- Starts Airflow services (Webserver, Scheduler, Init)
...
├─────────────────────────────────────────────────────────┤
│  ┌───────────┐  ┌─────────────┐  ┌──────────────────────┐ │
│  │  Airflow  │  │  PostgreSQL │  │    ETL (PySpark)     │ │
│  │   :8080   │  │    :5432    │  │  Python 3.12 + Java  │ │
│  │           │  │             │  │                      │ │
│  └───────────┘  └─────────────┘  └──────────────────────┘ │
│       │              │                    │              │
│       └──────────────┴────────────────────┘              │
...
python -m dev.etl.jobs.bronze.taxi_ingestion_job --taxi-type green --year 2024 --month 1
...
python -m dev.etl.jobs.bronze.taxi_ingestion_job --help
```
...
python -m dev.etl.jobs.gold.taxi_gold_job --taxi-type green --year 2024 --month 1

# Show help for all options
python -m dev.etl.jobs.gold.taxi_gold_job --help
```
...
python -m dev.etl.jobs.load.postgres_load_job --taxi-type green

# Show help for all options
python -m dev.etl.jobs.load.postgres_load_job --help
```
...
docker-compose exec etl python -m dev.etl.jobs.bronze.taxi_ingestion_job --taxi-type yellow --start-year 2025 --start-month 1 --end-year 2025 --end-month 11

# Zone lookup ingestion
docker-compose exec etl python -m dev.etl.jobs.misc.zone_lookup_ingestion_job
...
```bash
# Ingestion
docker exec nyc-taxi-etl-etl python -m dev.etl.jobs.bronze.taxi_ingestion_job --taxi-type yellow --year 2024 --month 1

# Transformation
docker exec nyc-taxi-etl-etl python -m dev.etl.jobs.gold.taxi_gold_job --taxi-type yellow --year 2024 --month 1

# Load
docker exec nyc-taxi-etl-etl python -m dev.etl.jobs.load.postgres_load_job --taxi-type yellow --year 2024 --month 1
```
...
1. Open GCS Console (Google Cloud Console)
2. Browse the `nyc-taxi-etl` bucket
...
docker exec -it nyc-taxi-etl-etl python -m dev.etl.jobs.bronze.taxi_ingestion_job --help
```
