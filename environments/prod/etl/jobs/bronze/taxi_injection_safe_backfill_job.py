...
def check_partition_exists(
    spark, taxi_type: str, year: int, month: int, config
) -> Tuple[bool, int, int]:
    """
...
def delete_partition(spark, taxi_type: str, year: int, month: int, config) -> bool:
    """
...
def safe_backfill_month(
    ...
    """
    from environments.prod.etl.jobs.bronze.taxi_ingestion_job import TaxiIngestionJob
    ...
    exists, total_before, unique_before = check_partition_exists(
        spark, taxi_type, year, month, config
    )
    ...
    if exists:
        ...
        if delete_existing:
            ...
            if delete_partition(spark, taxi_type, year, month, config):
...
def safe_historical_backfill(
    ...
    """
    from environments.prod.etl.jobs.utils.spark_manager import SparkSessionManager
